...
SPEC = {
    "nodes": [
        {
            ...
            "xray": {
                "gotcha": "Plans travel in the loop's system message, so every researcher pass re-reads them. Drift shows up as reflect bouncing the draft.",
                "q": [],
            },
        },
        {
            ...
            "xray": {
                "gotcha": "Hand-rolled loop instead of create_agent because it needs a second entrance: revise re-enters HERE with critique in the same message list.",
                "q": ["Loop exit? Router reads the last message: tool_calls means tools, none means draft."],
            },
        },
        {
            "id": "tools", "label": "Tools", "role": "desk, sql, math",
            "xray": {
                "gotcha": "ToolNode with handle_tool_errors=True: a crash becomes a ToolMessage the model can react to, not an exception.",
                "q": ["Why a calculator? Token prediction is not arithmetic. Every number stays checkable."],
            },
        },
        {
            "id": "draft", "label": "Draft", "role": "writes the answer",
            "xray": {
                "gotcha": "Reads the FULL loop history, so no number gets summarized away before it is used.",
                "q": [],
            },
        },
        {
            ...
            "quiet": True,
            "xray": {
                "gotcha": "Structured verdict: is every claim backed by a fetched number? Mechanical enough that self-review actually works.",
                "q": ["Why cap at 2 revisions? Value falls off a cliff after the first bounce."],
            },
        },
        {
            "id": "revise", "label": "Revise", "role": "injects critique",
            "xray": {
                "gotcha": "Re-enters RESEARCH, not rewriting: a bounced draft goes and fetches the missing number instead of rephrasing around the hole.",
                "q": [],
            },
        },
        {
            ...
            "xray": {
                "gotcha": "`report` set = the gate passed. Downstream trusts the key, never re-derives the verdict.",
                "q": [],
            },
        },
    ],
    ...
    "state": [
        ...
        {"key": "plan",      "kind": "overwrite", "note": "steps mapped to tools"},
        {"key": "messages",  "kind": "append",
         "note": "add reducer: both loops share one history"},
        ...
        {"key": "report",    "kind": "overwrite", "note": "set only by finalize"},
    ],
    "framework": [
        {"api": "two nested loops", "note": "tool cycle inside reflection cycle"},
        {"api": "model.bind_tools", "note": "router reads tool_calls"},
        {"api": "ToolNode(handle_tool_errors)", "note": "crashes become ToolMessages"},
        {"api": "with_structured_output(Verdict)", "note": "the gate routes on a bool"},
        {"api": "Annotated[list, operator.add]", "note": "both loops append, no overwrites"},
